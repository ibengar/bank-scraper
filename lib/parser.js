...
class ScraperBank {
  ...
  async getBCAMutation(tglawal, blnawal, tglakhir, blnakhir, res) {
    const browser = await puppeteer.launch(this.konfigbrowser);
    const page = await browser.newPage();
    ...
    try {
      ...
      // const element2 = await newPage.$('table:nth-child(4) > tbody > tr > td > input:nth-child(1)');
      // console.log(element2)

      // const textExists = await newPage.evaluate(() => {
      //   return document.querySelectorAll('table:nth-child(4) > tbody > tr:nth-child(2) > td')[0].innerHTML
      // });
      // console.log(textExists);

      // const myButton = await newPage.$('button[type="submit"]');
      // console.log(myButton)

      await newPage.click(
          // "table:nth-child(4) > tbody > tr > td > input:nth-child(1)"
          `table:nth-child(4) > tbody > tr:nth-child(2) > td > input:nth-child(1)`
      );
