...
class ScraperBank {
  ...
  async getBCAMutation2(tglawal, blnawal, tglakhir, blnakhir, res) {
    const browser = await puppeteer.launch(this.konfigbrowser);
    const page = await browser.newPage();
    ...
    try {
      ...
      // Wait for the specified time interval before navigating to the next page
      ...
      const value = await newPage.evaluate(() => {
            // return document.querySelectorAll('table:nth-child(3) > tbody > tr:nth-child(2) > td:nth-child(1) > div > font')[0].innerHTML
            return document.querySelectorAll('table:nth-child(4) > tbody > tr:nth-child(2) > td')[0].innerHTML
          }
      );
      console.log(value)
      await newPage.click(
      ...
      // await newPage.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 3000 });

      // await new Promise(resolve => setTimeout(resolve, 5000));

      await newPage.screenshot({path: 'mutation-fill.png'});
      //
      // const element2 = await newPage.$('tr');
      // console.log(element2)


      // await new Promise(resolve => setTimeout(resolve, 20000));

      // await newPage.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 5000, });
      ...
      await Promise.all([
        ...
        // page.waitForSelector('#new-page-loaded')
      ]);
      ...
      // const value1 = await newPage.evaluate(() => {
      //       // return document.querySelectorAll('table:nth-child(3) > tbody > tr:nth-child(2) > td:nth-child(1) > div > font')[0].innerHTML
      //       return document.querySelectorAll('table:nth-child(4) > tbody > tr:nth-child(2) > td')[0].innerHTML
      //     }
      // );
      ...
      await newPage.screenshot({path: 'mutation-result.png'});
      ...
      // await newPage.waitForSelector('table[bordercolor="#ffffff"] tr', {timeout: 2000});
  ...
  async getBCAMutationMultiAccount(tglawal, blnawal, tglakhir, blnakhir, res) {
    const browser = await puppeteer.launch(this.konfigbrowser);
    const page = await browser.newPage();
  ...
  async getBCAMutation(tglawal, blnawal, tglakhir, blnakhir) {
    const browser = await puppeteer.launch(this.konfigbrowser);
    const page = await browser.newPage();
    // await page.evaluateOnNewDocument(() => {
    //   delete navigator.__proto__.webdriver;
